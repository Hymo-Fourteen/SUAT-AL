...
def device_context(device):
    """Return a CUDA device context or a no-op context for CPU."""
...
def setup_classification_model(
    ...
    with device_context(device):
        if training_config.get("ensemble", False) is True:
            task_model = classification_ensemble_factory(dataset_config, method, training_config, device)
        else:
            task_model = classification_model_factory(dataset_config, method, training_config, device)

        models = {"task": task_model}
        if is_loss_learning(method):
            feature_sizes = dataset_config["feature_sizes"][training_config["model"]]
            loss_module = GenericLossNet(feature_sizes, task_model.num_channels).to(device)
            ...
            detector = classification_model_factory(detector_dataset_config, method, training_config, device)
...
def load_pretrained_checkpoint(
    ...
    checkpoint_loaded = torch.load(os.path.join(model_name), weights_only=False)
...
def classification_ensemble_factory(
    ...
    for _ in range(training_config["ensemble_size"]):
        task_model = classification_model_factory(dataset_config_copy, method, training_config, device)
...
def classification_model_factory(
    ...
    elif training_config["model"] == "Resnet18T":
        # torchvision-style ResNet18; honors the pretrained flag and uses
        # an ImageNet-pretrained backbone with a trainable classification head.
        task_model = torchResnet.ResNet18(
