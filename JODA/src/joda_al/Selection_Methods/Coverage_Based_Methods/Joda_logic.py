...
class JodaLogic():
    ...
    def assess(self, data_loader, gain_mode="standard", surprise_strategy="surprise"):
        ...
        cached_dataloader = DataLoader(cached_dataset, batch_size=128, shuffle=False, num_workers=0)
    ...
    def nac_forward_pass(self, data_batch, label_batch, surprise_strategy="surprise", use_prediction=False):
        """Forward pass used by JODA's coverage/assess/top-k stages."""
        SA_batch = []
        if self.use_nac:
            layer_output_dict, score = get_layer_output_nac(self.model, data_batch,
                                                            layer_selection=self.layer_selection,
    ...
    def build_greedy(self, data_loader, use_prediction=False, build=True):
        """Cache NAC activations for the current (labeled or unlabeled) pool."""
        ...
        for i, (data, label) in enumerate(tqdm(data_loader)):
            if isinstance(data, tuple):
                ...
                data = data.to(self.device)
            label = label.to(self.device)
            SA_batch, label_batch, layer_output_dict, score = self.nac_forward_pass(
                data, label, surprise_strategy="surprise", use_prediction=use_prediction
            )
            n = label_batch.size(0)
            idcs = torch.arange(offset, offset + n, dtype=torch.long)
            offset += n
            ...
            cached_sample_idcs = torch.cat((cached_sample_idcs, idcs), dim=0)
            cached_true_labels = torch.cat((cached_true_labels, label.detach().cpu().long()), dim=0)
    ...
    def calculate_greedy(self, SA_batch, label_batch, score, surprise_strategy="surprise"):
        """Return JODA coverage gains and per-sample statistics.

        Statistics columns: [dist_a, dist_b, dsa, energy, gen].
        """
        cove_set = set()
        dsa_list = []
        all_dist_a = []
        ...
        for i, label in enumerate(label_batch):
            SA = SA_batch[i]
            if self.use_shapley:
                dist_a_list_batch = torch.linalg.norm(
                    self.shapley_dict[int(label.cpu())].to(self.device) *
                    (torch.from_numpy(SA).to(self.device) - torch.from_numpy(self.SA_cache[int(label.cpu())]).to(self.device)),
                    dim=1
                )
            else:
                dist_a_list_batch = torch.linalg.norm(
            ...
            if self.weight[1] == 0:
                idx_a = torch.topk(dist_a_list_batch, 2, dim=0, largest=False)[1][1]
            else:
                idx_a = torch.argmin(dist_a_list_batch, 0).item()

            (SA_a, dist_a) = (self.SA_cache[int(label.cpu())][idx_a], dist_a_list_batch[idx_a])
            ...
            dsa = min(self.weight[0], 1.0) * ((energy[i] - self.weight[1]) / self.std[1]) +                   max(1.0 - self.weight[0], 0.0) * ((dsa - self.weight[0]) / self.std[1])
            if surprise_strategy == "dist_a":
                gain = float(dist_a)
            elif surprise_strategy == "dist_b":
                gain = float(1.0 / dist_b) if dist_b > 0 else 1e-6
            else:
                gain = dsa
            dsa_list.append(gain)
            all_dist_a.append(float(dist_a))
            all_dist_b.append(float(dist_b))
            energy_list.append(float(energy[i]))
            gen_list.append(0.0)
            if (not np.isnan(dsa)) and (not np.isinf(dsa)):
                cove_set.add(int(dsa / self.threshold))
        self.coverage_set = self.coverage_set.union(cove_set)
        stats = torch.tensor([all_dist_a, all_dist_b, dsa_list, energy_list, gen_list], dtype=torch.float).t()
        return torch.tensor(dsa_list, dtype=torch.float), stats
