...
def create_argparser(
    ...
    for parameter_dict in [remove_defauts(load_training_config(),defaults_to_keep), experiment_config]:
        ...
        for k, al in parameter_dict.items():
            if k not in types_s:
                # Some dataclass fields (e.g. args, model_configuration) have no
                # CLI type mapping; keep their defaults and skip the CLI option.
                continue
            option_string = f"--{k}"
            if option_string in seen_options:
                # A field can appear in both the training and experiment
                # config sections; register it only once.
                continue
            ...
            if types_s[k] is bool:
                ...
                parser.add_argument(
                    option_string, default=al, type=types_s[k]
                )
    return parser
